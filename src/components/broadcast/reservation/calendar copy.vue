          ...
          <template #week="{ scope: { week, weekdays } }">
            ...
            >
              ...
              >
                ...
                >
                  {{ computedEvent.event.name + (computedEvent.event.time ? ' - ' + computedEvent.event.time : '') }}
                  <q-tooltip>{{ computedEvent.event }}</q-tooltip>
                </div>
              </div>
            </template>
          </template>
...
export default {
  ...
  setup () {
    ...
    const eventlist = computed(() => {
      ...
      schedules.forEach(schedule => {
        ...
        if (schedule.repeat === '매일') {
          temp = { ...schedule }
          temp.start = getCurrentDay(1)
          temp.end = getCurrentDay(31)
          temp.bgcolor = 'green'
          mapSchedules.push(temp)
        } else if (schedule.repeat === '한번') {
          temp = { ...schedule }
          temp.start = temp.date
          temp.end = temp.date
          temp.bgcolor = 'blue'
          mapSchedules.push(temp)
        }
    ...
    function getWeekEvents (week) {
      ...
      const eventsWeek = []
      eventlist.value.forEach((event, id) => {
